...
@param Show Front
@desc 画像の表示位置Z座標です。 メッセージウィンドウより前面に表示する場合はtrueを指定してください。
@default false
...
@param Default Anchor X
@desc ピクチャのアンカー位置Xです。 デフォルトは0.5(画像の中心)です。0~1.0で指定してください。
@default 0.5

@param Default Anchor Y
@desc ピクチャのアンカー位置Yです。 デフォルトは1.0(画像の下端)です。0~1.0で指定してください。
@default 1.0

@param Default Opacity
@desc ピクチャの透明度の初期値です。 デフォルトは0(非表示)です。0~255で指定してください。
@default 0

@param Proxy Variable ID
@desc \PVの制御文字がこの数値と置き換わります。 同じマクロを使いまわしたり、汎用性を上げるための仕組みです。
@default 1

@param Cursor Name
@desc スプライト選択時にカーソルとして使用する画像名です。 img/system/内に指定した画像を配置してください。
@default cursor
@type file
@require 1
@dir img/system/
