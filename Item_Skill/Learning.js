 ...
 * と記述します。
 * 
 * そのスキルをラーニング能力を持ったアクターが受けたとき、または、設定によっては
 * 見るだけで習得率%の確率で設定したIDのスキルを習得します。
 * ラーニング能力は特徴オブジェクト(アクター,クラス,装備,ステートなど)のメモ欄に
 * <ラーニング能力>
 * と記述することで設定することができます。
 * 
 ...
 * ログにラーニングの表示が出た後、次の文字カラーがラーニング時のカラーになるバグを修正。
        ...
        for (var i=0;i<length;i++) {
            ...
            if (text.match(/[<＜](?:ラーニング|LEARNING)\s*(\d+)[,，](\d+)[%％]?[>＞]/)) {
                var obj = {'skill': Number(RegExp.$1), 'rate': Number(RegExp.$2), 'conditions':[],
                            'text':null, 'popup':null, 'se':null, 'infoText':null };
                var ts = [];
