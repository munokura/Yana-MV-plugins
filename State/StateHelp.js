...
/*:
@plugindesc ver1.03/Add help to states and a mechanism to view state details.
@author Yana
...
You can set a description for a state by writing the following in the state's Note field:

<STATE_HELP>
State Description
</STATE_HELP>

You can set a description for that state by writing the following in the state's Note field:

------------------------------------------------------
Specifications
-----------------------------------------------------

During battle, when an actor command or party command is active, you can call up the state help window by clicking the left side of the screen.

This setting can be disabled by setting UseTouchCall to false.

The state help window can also be closed by clicking outside the state help window.
...
Fixed a bug that caused enemies to appear when invoked from the menu.
ver. 1.02:
Fixed processing to prevent conflicts with YEP_X_BattleSysATB.
ver. 1.01:
Fixed a bug that caused hidden enemies to appear.
ver. 1.00:
Released
*/
