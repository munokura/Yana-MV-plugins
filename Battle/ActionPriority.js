...
/*:
@plugindesc ver1.01/Setting priorities for actions You can now set skills and items with Traits and priorities.
@author Yana
...
Note
------------------------------------------------------
When installing this plugin, the behavior of multiple actions will be changed.
Even in the case of multiple actions, actions will be activated according to the activation speed of each action.

*When using in conjunction with YEP_BattleEngineCore, place this below.
...
Write
<Priority:x>
in the Note field of an object, skill, item, etc. with a Traits.

When setting it for a Traits, the priority of the battler with that Traits will be set to x.
When setting it for a skill or item, the priority of that skill or item will be set to x.

Setting the priority to + will speed up the action, while setting it to - will slow down the action.
The priority of Traits, item, and skill is determined by their combined values.

Also, if ConditionallyCore is installed, you can set conditions for priority activation.

To add conditions, write:
...
Added compatibility with pre-action commons.
ver. 1.00:
Released
*/
