...
/*:
@plugindesc ver1.00/Adds a command to change equipment during battle.
@author Yana
...
Install the plugin and set AddBattleEquip to true in the plugin parameters to activate it.

Note:
When using with BattleLayout-SaGa, place this plugin on top.
When using with EquipAndShopStatusR, place this plugin on bottom.

- Setting Using Notes
If you write either of the following in the note for an object with a characteristic:
<ChangeableBattleEquip:○,○,○…>
, actors with that characteristic will be able to change equipment with the ○,○,○… equipment types.
In this case, this setting takes precedence over the plugin parameter setting.

Similarly, if you specify any of the following:
<UnchangeableBattleEquip:○,○,○…>
, actors with that Traits will be unable to change equipment with the ○,○,○… equipment types.
In this case, this setting takes precedence over the plugin parameter settings.
...
@desc This setting determines whether to add an equip command to the actor commands. Specify true/false.
@default true

@param EquipSkillTypeId
@desc This name will be used for the skill type that will be synchronized with equipment changes. If this skill type is sealed, you will not be able to change equipment.
@default 6

@param BattleChangeableEquipTypes
@desc This is the part of the body that can be equipped during battle. Please specify the equipment type, separated by commas.
@default 1,2
*/
...
@param AddBattleEquip
@desc アクターコマンドに装備コマンドを追加するかの設定です。 true/falseで指定してください。
@default true

@param EquipSkillTypeId
@desc 装備変更と同期させるスキルタイプで、この名称が使用されます。 このスキルタイプが封印されていると装備変更ができません。
@default 6

@param BattleChangeableEquipTypes
@desc 戦闘中に装備変更可能な部位です。 装備タイプを,で区切って指定してください。
@default 1,2
*/
