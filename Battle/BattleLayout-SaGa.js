...
Changed the help displayed in the mini window to recognize \n.
ver1.05:170110
...
@param CommandIcons
@desc コマンドのアイコンインデックスです。 1:基本、2:拡張、3:アイテム、4以降はスキルタイプ1番目から順です。
@default 76,209,83,79,78
...
@param ItemCommandUsable
@desc アイテムコマンドを使用するかの設定です。 falseを指定すると、アイテムコマンドがなくなります。
@default true
@type boolean
...
@param ShowActorActions
@desc ステータスに行動回数を表示するかの設定です。 表示する場合はアイコンインデックスを指定してください。
@default 16
...
@param MoveCameraX
@desc カメラのX方向の動作範囲です。 数値を大きくすると、背景が見切れます。
@default 92
@type number
...
@param ShowEnemyAtbGauge
@desc エネミーのATBゲージを表示するかの設定です。 true/falseで指定してください。
@default true
@type boolean
